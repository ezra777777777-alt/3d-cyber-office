...
8. 用户能打开至少一组日历/计划数据并查看进度。
9. 用户能看到日志、文件、Cron Job 和 Gateway 状态基础视图。
10. 用户能查看一条每日复盘或改进提示。
11. 用户能对至少一个本地可确认事项执行完成动作。
12. 任务事件协议存在清晰边界，后续可接外部适配器。
...
7. 附加工作台中，日历、文件、Cron Job 和 Gateway 哪个最先接真实数据源。
8. 每日复盘第一版只展示复盘卡片，还是要支持用户主动触发一次回顾。
