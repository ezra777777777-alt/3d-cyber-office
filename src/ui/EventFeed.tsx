...
import { labelForEventType } from '@/i18n/zh';
...
export function EventFeed() {
  ...
  return (
    <div className="cyber-panel m-2 p-2 flex flex-col max-h-48 overflow-hidden">
      <div className="text-gray-500 text-xs font-medium mb-2 px-1">事件流</div>
      <div className="flex-1 overflow-y-auto space-y-0.5">
        {events.length === 0 && (
          <div className="text-gray-600 text-xs px-1 py-2 italic">暂无事件。启动演示后会看到任务流。</div>
        )}
        {events.map((evt) => {
          ...
          return (
            <div key={evt.id} className="flex gap-2 items-start text-xs hover:bg-white/5 rounded px-1 py-0.5">
              ...
              <span className="text-gray-300 truncate">{labelForEventType(evt.type)}</span>
              ...
              {evt.payload?.action === 'complete' && (
                <span className="text-cyber-success truncate">已完成：{String(evt.payload.title || '').slice(0, 30)}</span>
              )}
